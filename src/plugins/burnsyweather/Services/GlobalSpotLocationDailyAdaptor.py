...
class GlobalSpotLocationDailyAdaptor:
    def get_spot_daily_forecast(self, icons_path,lat, long):
        ...
        for i in range(2,5):
            global_spot_location_daily[f"day_{i-1}_weather_symbol"] = os.path.join(icons_path, f'{weather_data.features[0].properties.timeSeries[i].daySignificantWeatherCode}.svg')
